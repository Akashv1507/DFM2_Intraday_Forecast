...
class ForecastedDemandFetchForRevisionRepo():
    ...
    def fetchForecastedDemandForRevision(self, start_Time: dt.datetime, end_Time: dt.datetime, entityTag:str, avgBiasError: float) -> List[Tuple]:
        ...
        # currdate = dt.datetime.strptime("2021-01-21 22:40:40", '%Y-%m-%d %H:%M:%S').replace(hour =0 ,minute =0,second=0, microsecond=0)
        ...
        # if startExceptionTime <= dt.datetime.strptime("2021-01-21 22:40:40", '%Y-%m-%d %H:%M:%S') < endExceptionTime:
        ...
        try: 
            ...
            try:
                ...
                # fetch_sql = "SELECT time_stamp, entity_tag, forecasted_demand_value FROM dfm2_forecast_revision_store WHERE time_stamp BETWEEN TO_DATE(:start_time,'YYYY-MM-DD HH24:MI:SS') and TO_DATE(:end_time,'YYYY-MM-DD HH24:MI:SS') and entity_tag =:entity and revision_no = 'R0A' ORDER BY time_stamp"
                fetch_sql = "SELECT time_stamp, entity_tag, forecasted_demand_value FROM dfm2_dayahead_demand_forecast WHERE time_stamp BETWEEN TO_DATE(:start_time,'YYYY-MM-DD HH24:MI:SS') and TO_DATE(:end_time,'YYYY-MM-DD HH24:MI:SS') ORDER BY time_stamp"
