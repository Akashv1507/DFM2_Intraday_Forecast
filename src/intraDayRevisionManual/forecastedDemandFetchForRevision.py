...
class ForecastedDemandFetchForRevisionRepo():
    ...
    def fetchForecastedDemandForRevision(self, start_Time: dt.datetime, end_Time: dt.datetime, entityTag:str, avgBiasError: float) -> List[Tuple]:
        ...
        if startExceptionTime <= currTime < endExceptionTime:
            ...
            fetch_sql = "SELECT time_stamp, entity_tag, forecasted_demand_value FROM dfm2_dayahead_demand_forecast WHERE time_stamp BETWEEN TO_DATE(:start_time,'YYYY-MM-DD HH24:MI:SS') and TO_DATE(:end_time,'YYYY-MM-DD HH24:MI:SS') and entity_tag =:entity ORDER BY time_stamp"
